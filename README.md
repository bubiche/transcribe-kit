...
- **Standalone Process tab** — apply AI prompt templates to any text (transcripts, notes, or pasted content) — no transcription required first. Ships with built-in templates (cleanup, meeting notes, summary), supports custom user templates with `{{transcript}}` and `{{noteN}}` placeholders, and lets you reference saved notes as additional context. After the first run, keep iterating with chat-style follow-ups (⌘/Ctrl+Enter to send) and save either the last reply or the full thread as a note. Run locally with a bundled llama-server sidecar (no API key needed) or via any OpenAI-compatible API.
- **Notes** — persistent local notes. Transcripts auto-save into Notes when transcription completes; processed results save on demand from the Process tab (last reply, or the entire thread). Notes can be loaded back into Process as input or referenced from templates as `{{noteN}}` slots.
